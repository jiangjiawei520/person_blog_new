...
复制代码
```
输入上面 `awk` 命令打印后得到这样的结果：![](https://cdn.nlark.com/yuque/0/2021/png/12484160/1626516405864-d586b09f-20c0-4ec9-9154-c392721f2a60.png#align=left&display=inline&height=50&originHeight=50&originWidth=1109&size=0&status=done&style=none&width=1109)不再像使用 `print` 命令会默认以换行符作为默认的输出了，而是全部集中在一行，因此使用 `printf` 需要自定义输出格式：
```
awk 'BEGIN{FS=":"}{printf "%s\n",$1}' /etc/passwd # %s表示打印字符串 \n表示换行
复制代码
```
![](https://cdn.nlark.com/yuque/0/2021/png/12484160/1626516405860-4ef7ceca-5105-4c90-9df9-60339cb342e8.png#align=left&display=inline&height=369&originHeight=369&originWidth=632&size=0&status=done&style=none&width=632)
...
![](https://cdn.nlark.com/yuque/0/2021/png/12484160/1626516405847-593add64-12bb-4e6a-b27e-f35b0507d4cb.png#align=left&display=inline&height=369&originHeight=369&originWidth=626&size=0&status=done&style=none&width=626)
默认是右对齐，我们也可以添加修饰符，使其左对齐：
```
awk 'BEGIN{FS=":"}{printf "%-20s %-20s\n",$1,$7}' /etc/passwd
复制代码
```
![](https://cdn.nlark.com/yuque/0/2021/png/12484160/1626516405856-d953080c-f82e-4da1-b882-3902b1629c7e.png#align=left&display=inline&height=367&originHeight=367&originWidth=625&size=0&status=done&style=none&width=625)
其它示例：
```
...
作者：Lion
链接：[https://juejin.cn/post/6935365727205457928](https://juejin.cn/post/6935365727205457928)
来源：掘金
