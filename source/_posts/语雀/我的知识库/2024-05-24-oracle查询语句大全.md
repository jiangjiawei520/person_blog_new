...
---
0.create tablespace test_work datafile'/u01/app/oracle/oradata/abc.dbf' size 10M AUTOEXTEND ON; //创建数据文件

1.create user username identified by password;//创建用户，建用户名和密码oracle ,oracle

2.grant connect,resource,dba to username;//授权 grant connect,resource,dba,sysdba to username;

3.connect username/password //连接数据库。
<!--more-->
