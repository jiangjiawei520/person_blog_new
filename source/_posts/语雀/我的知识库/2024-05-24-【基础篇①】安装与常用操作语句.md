...
bsondump 导出BSON结构
<!--more-->
mongo 客户端，客户端程序，连接MongoDB
mongod 服务端，服务端程序，启动MongoDB
mongodump 整体数据库二进制导出，备份程序
mongoexport 导出易识别的json文档或csv文档，数据导出程序
mongorestore 数据库整体导入，数据恢复程序
mongos 路由器(分片用)，数据分片程序，支持数据的横向扩展
