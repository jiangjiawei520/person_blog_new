...
![image.png](https://cdn.nlark.com/yuque/0/2021/png/12484160/1632145657557-9ede1927-cba6-462f-87c5-cc37f256dd67.png#clientId=u58be6ff4-bc7a-4&from=paste&id=ud83353d9&originHeight=812&originWidth=793&originalType=url&ratio=1&size=408103&status=done&style=none&taskId=uf2a1d8db-f8e2-4504-a362-3c54d19f1c9)
**先从比较有代表性的两个MQ(rabbitMQ,kafka)，功能对比(图还是Q来的)**
![image.png](https://cdn.nlark.com/yuque/0/2021/png/12484160/1632145656574-cf320798-db2e-4446-81cd-45b87aa290c2.png#clientId=u58be6ff4-bc7a-4&from=paste&id=uef7e2b10&originHeight=788&originWidth=859&originalType=url&ratio=1&size=270093&status=done&style=none&taskId=u1485e306-0b6d-421b-bcd3-84837adae1d)
![image.png](https://cdn.nlark.com/yuque/0/2021/png/12484160/1632145657727-c7ef861a-e285-486f-a4f9-cb4845b63cb7.png#clientId=u58be6ff4-bc7a-4&from=paste&id=u6532b97d&originHeight=785&originWidth=861&originalType=url&ratio=1&size=451419&status=done&style=none&taskId=uf981b488-3602-4730-a911-b01378b1529)
![image.png](https://cdn.nlark.com/yuque/0/2021/png/12484160/1632145654666-924faf98-4666-4e3f-9695-2ee2742e9254.png#clientId=u58be6ff4-bc7a-4&from=paste&id=u17f1a98e&originHeight=420&originWidth=863&originalType=url&ratio=1&size=175450&status=done&style=none&taskId=u97d33aa9-94da-48b5-b6fd-10689f9f485)
**应用方面：**
...
![image.png](https://cdn.nlark.com/yuque/0/2021/png/12484160/1632145656367-7a06a3d3-2a9c-443f-b076-d63edcdb1fcf.png#clientId=u58be6ff4-bc7a-4&from=paste&id=ub4cc39f1&originHeight=757&originWidth=1428&originalType=url&ratio=1&size=290456&status=done&style=none&taskId=u384e8bfb-5b13-418e-a5b3-7325241fe1d)
#### kafka
几个重要概念：
...
下载地址：[kafka.apache.org/downloads](https://link.juejin.cn?target=https%3A%2F%2Fkafka.apache.org%2Fdownloads)
# 下载解压 mkdir kafka && cd kafka wget http://mirrors.shuosc.org/apache/kafka/1.0.0/kafka_2.11-1.0.0.tgz tar -xzf kafka_2.11-1.0.0.tgz cd kafka_2.11-1.0.1 # 启动zookeeper bin/zookeeper-server-start.sh config/zookeeper.properties # 启动kafka bin/kafka-server-start.sh config/server.properties # 创建Topic test bin/kafka-topics.sh --create --zookeeper localhost:2181 --replication-factor 1 --partitions 1 --topic test # 参数描述： # create: 创建Topic # zookeeper：zookeeper集群信息，多个用,分开 # replication-factor：复制因子 # partitions：分区信息 # topic：主题名  # 向topic发送消息 bin/kafka-console-producer.sh --broker-list localhost:9092 --topic test > 随便输入  # 向topic获取消息 bin/kafka-console-consumer.sh --bootstrap-server localhost:9092 --topic test --from-beginning # 就能看到前面输入的消息了 复制代码
Kafka监控工具
kafka没有自带的web ui,这里使用KafkaOffsetMonitor, 程序一个jar包的形式运行，部署较为方便。只有监控功能，使用起来也较为安全。
KafkaOffsetMonitor托管在Github上，可以通过Github下载。 下载地址：[github.com/Morningstar…](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2FMorningstar%2Fkafka-offset-monitor%2Freleases)
# 下载好之后cd到KafkaOffsetMonitor所在目录，可以直接启动，也可以编写shell脚本来启动 java -cp KafkaOffsetMonitor-assembly-0.4.1-SNAPSHOT.jar com.quantifind.kafka.offsetapp.OffsetGetterWeb --port 8089 --zk 127.0.0.1:2181  --refresh 5.minutes  --retain 1.day 复制代码
编写脚本启动
mkdir kafka-monitor.sh chmod  +x kafka-monitor.sh vi kafka-monitor.sh # 把之前启动的命令复制进来，如： #! /bin/bash java -Xms512M -Xmx512M -Xss1024K \      -cp KafkaOffsetMonitor-assembly-0.4.1-SNAPSHOT.jar \      com.quantifind.kafka.offsetapp.OffsetGetterWeb \      --zk localhost:2181 \      --port 8089 \      --refresh 10.seconds \      --retain 5.days # 保存，然后就可以启动脚本了 复制代码
...
**github上详细参数说明：**
![image.png](https://cdn.nlark.com/yuque/0/2021/png/12484160/1632145658724-69a4f33f-a18c-40ad-960e-33b9531f466a.png#clientId=u58be6ff4-bc7a-4&from=paste&id=ubf588b56&originHeight=750&originWidth=911&originalType=url&ratio=1&size=491305&status=done&style=none&taskId=ube925251-894d-4418-82b6-e474eb749ae)
![image.png](https://cdn.nlark.com/yuque/0/2021/png/12484160/1632145658722-13382777-b6c1-42fa-b458-3df9d0660dd9.png#clientId=u58be6ff4-bc7a-4&from=paste&id=uc45d7512&originHeight=564&originWidth=1434&originalType=url&ratio=1&size=189742&status=done&style=none&taskId=ue30200d0-809c-4e51-a910-fe80881a6da)
    Topic：订阅的主题     Partition:分区编号     Offest：表示该parition已经消费了多少条message     logSize:表示该partition已经写了多少条message     Lag：表示有多少条message没有被消费。     Owner：表示消费者     Created：该partition创建时间     Last Seen：消费状态刷新最新时间。 复制代码
**注意**
本机能访问，但其它机器不能访问应该就是防火墙的问题，开放对应端口，或者关闭防火墙
如果能访问，但是没有信息显示出来，只显示了黑色的屏，是因为ui页面所依赖的ajax.googleapis.com等公共库被墙了
[解决办法](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Fjustjavac%2FReplaceGoogleCDN)

作者：ohcomeyes
链接：https://juejin.cn/post/6844903626171760653
来源：掘金
